...
<div class="tile" style="margin-bottom:1rem;">
    <form method="GET" action="{{ route('attendance.monitoring-matrix') }}"
          style="display:flex;flex-wrap:wrap;gap:1rem;align-items:flex-end;">

        <div style="display:flex;flex-direction:column;gap:0.3rem;">
            <label for="mat-department" style="font-size:0.82rem;font-weight:600;color:#374151;">Department</label>
            <select id="mat-department" name="department_id"
                    style="padding:0.5rem 0.7rem;border:1px solid #d1d5db;border-radius:6px;font-size:0.9rem;background:#fff;min-width:220px;">
                @foreach($departments as $d)
                    <option value="{{ $d->Dept_id }}" @selected($d->Dept_id === $departmentId)>
                        {{ $d->Dept_name }}
                    </option>
                @endforeach
            </select>
...
<div class="tile" style="padding:0;overflow:hidden;">
    <div style="padding:1rem 1.25rem 0.75rem;border-bottom:1px solid #e5e7eb;">
        <div style="font-weight:700;font-size:0.95rem;text-transform:uppercase;letter-spacing:.03em;">
            {{ $dept?->Dept_name ?? 'Select a Department' }}
        </div>
...
function exportMonitoringMatrixExcel() {
    var typeFilter = document.getElementById('matrix-type-filter');
    var deptSelect = document.getElementById('mat-department');
    startExport('{{ route('export-jobs.create') }}', {
        type: 'monitoring_matrix',
        params: {
            month: {{ $month }},
            year: {{ $year }},
            employee_type: typeFilter ? typeFilter.value : '',
            department_id: deptSelect ? parseInt(deptSelect.value, 10) : null,
        },
