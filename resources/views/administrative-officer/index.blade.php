@extends('dashboards.layout', [
    ...
    'subtitle' => 'Department Management Overview',
])
        ...
        modalOverlay.setAttribute('aria-hidden', 'false');
            ...
            try {
                if (type === 'workforce') {
                    ...
                    rows.forEach(r => {
                        table += `<tr><td>${r.EmpNo || ''}</td><td>${r.name || ''}</td><td>${r.position || ''}</td><td><span style="display:inline-block;padding:4px 8px;border-radius:6px;background:#16a34a;color:#fff;font-weight:700;font-size:0.82rem">${r.status || 'In Office'}</span></td></tr>`;
                    });
                    table += '</tbody></table>';
