...
class SalaryMatrixSecondTrancheSeeder extends Seeder
{
    public function run(): void
    {
        // 2025 Second Tranche - Executive Order No. 64, s. 2024 (NBC No. 601)
        // Effective January 1, 2025. SG 33 has only 2 steps.
        $table = [
        ...
        $ordinance = 'EO No. 64, s. 2024 (NBC No. 601) - Second Tranche';

        foreach ($table as $sg => $steps) {
            foreach ($steps as $index => $amount) {
                SalaryMatrix::updateOrCreate(
                    ['sg' => $sg, 'step' => $index + 1, 'effective_date' => '2025-01-01'],
                    ['amount' => $amount, 'ordinance_reference' => $ordinance]
                );
            }
        }

        $this->command->info('2025 Second Tranche salary matrix seeded: '.array_sum(array_map('count', $table)).' rows.');
    }
}
